...
void CameraController::update(double delta_time){
    ...
    if(pitch < -glm::half_pi<float>() * 0.99f) pitch = -glm::half_pi<float>() * 0.99f;
    if(pitch >  glm::half_pi<float>() * 0.99f) pitch  = glm::half_pi<float>() * 0.99f;
    yaw = glm::wrapAngle(yaw);// rotation around y-axis
    ...
    //change in the matrix by yaw and pirch to change the rotaion
    this-> direction=(glm::vec3(glm::cos(yaw), 0, -glm::sin(yaw)) * glm::cos(pitch) + glm::vec3(0, glm::sin(pitch), 0));
    //mat=glm::lookAt(this->eye, this->eye + this->direction, this->up);
    /// and Aya
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////
}
